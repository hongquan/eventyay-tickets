...
def answer_file_path(instance, filename):
    return path_with_hash(filename, base_path=f'{instance.question.event.slug}/question_uploads/')


class TalkQuestionManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().exclude(active=False).exclude(target=TalkQuestionTarget.REVIEWER)
...
class TalkQuestion(OrderedModel, PretalxModel):
    ...
    event = models.ForeignKey(to='Event', on_delete=models.PROTECT, related_name='talkquestions')
    ...
    freeze_after = models.DateTimeField(
        ...
        help_text=_('Set a deadline to stop changes to responses after the given date.'),
    )
    ...
    tracks = models.ManyToManyField(
        ...
        help_text=_('You can limit this field to some tracks. Leave empty to apply to all tracks.'),
    ...
    submission_types = models.ManyToManyField(
        to='SubmissionType',
        related_name='questions',
        help_text=_('You can limit this field to some session types. Leave empty to apply to all session types.'),
    ...
    max_length = models.PositiveIntegerField(
        ...
        help_text=_('Maximum allowed text length in characters or words (set in CfP settings).'),
    )
    ...
    max_datetime = models.DateTimeField(null=True, blank=True, verbose_name=_('Maximum value'))
...
class AnswerOption(PretalxModel):
    ...
    question = models.ForeignKey(to='TalkQuestion', on_delete=models.PROTECT, related_name='options')
...
class Answer(PretalxModel):
    ...
    question = models.ForeignKey(to='TalkQuestion', on_delete=models.PROTECT, related_name='answers')
    ...
    options = models.ManyToManyField(to='AnswerOption', related_name='answers')
